...
public class Player {
    ...
    // Chọn bài để đánh
    public List<Card> selectCardsToPlay(List<Card> selectedCards) {
        if (hand.containsAll(selectedCards)) {
            return selectedCards; // Nếu bài chọn nằm trong tay, trả về danh sách bài đó
        } else {
            throw new IllegalArgumentException("Selected cards are not in the player's hand.");
        }
    }
    ...
    // Phương thức để lấy danh sách rank
